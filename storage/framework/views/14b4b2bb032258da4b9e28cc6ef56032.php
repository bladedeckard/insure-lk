<div>
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="<?php echo e(route('policies.create')); ?>"
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50/50 transition-colors">
                    <td class="px-6 py-4">
                        <span class="text-sm font-medium text-gray-900 font-mono"><?php echo e($p->number ?? '—'); ?></span>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!$p->number): ?>
                            <div class="text-xs text-gray-400">Черновик</div>
                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                    </td>
                    <td class="px-6 py-4">
                        <span class="text-sm text-gray-700"><?php echo e($p->product->name); ?></span>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <span class="text-sm font-semibold text-gray-900"><?php echo e(number_format($p->premium, 2, ',', ' ')); ?> ₽</span>
                    </td>
                    <td class="px-6 py-4 text-center">
                        ...
                        ?>
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium <?php echo e($s['bg']); ?> <?php echo e($s['text']); ?> border <?php echo e($s['border']); ?>">
                    ...
                    <td class="px-6 py-4">
                        <span class="text-sm text-gray-500"><?php echo e($p->created_at->format('d.m.Y')); ?></span>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <a class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-primary-600 hover:bg-primary-50 transition-colors"
                               href="<?php echo e(route('policies.edit', $p)); ?>">
                                Открыть
                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/>
                                </svg>
                            </a>
                            <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('policies.delete')): ?>
                                <button onclick="confirmAction({
                                    type: 'danger',
                                    title: 'Удаление полиса',
                                    message: 'Вы уверены, что хотите удалить полис <?php echo e($p->number ?? '#' . $p->id); ?>? Это действие нельзя отменить.',
                                    confirmText: 'Удалить',
                                    onConfirm: function() { window.Livewire.find('<?php echo e($_instance->getId()); ?>').call('delete', <?php echo e($p->id); ?>); }
                                })"
